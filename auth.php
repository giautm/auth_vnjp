 ...
 */
class auth_plugin_vnjp extends auth_plugin_base
{
    ...
    function __construct()
    {
        ...
        $this->authtype = 'vnjp';
        $this->config = get_config('auth/vnjp');
        if (empty($this->config->extencoding)) {
            $this->config->extencoding = 'utf-8';
        }
    }
    function vnjp_encrypt_password($password, $passwordFormat, $passwordSalt)
    {
        if ($passwordFormat == 0) {
            return $password;
        } else if ($passwordFormat == 1) {
            return base64_encode(sha1(base64_decode($passwordSalt) . $password, TRUE));
        }

        return FALSE;
    }

    function vnjp_check_login($extusername, $password, $fromdb)
    {
        $authdb = $this->db_init();

        $rs = $authdb->Execute("SELECT PasswordFormat as format, PasswordSalt as salt
                               FROM {$this->config->table}
                               WHERE {$this->config->fielduser} = '".$this->ext_addslashes($extusername)."'");
        if (!$rs) {
            $authdb->Close();
            debugging(get_string('auth_dbcantconnect','auth_vnjp'));
            return false;
        }
        ...
        return hash_equals($fromdb, $this->vnjp_encrypt_password($password, $format, $salt));
    }
    ...
    function user_login($username, $password)
    {
        ...
        if ($this->is_internal()) {
            ...
            } else if ($this->config->passtype === 'vnjp') {
                return $this->vnjp_check_login($extusername, $extpassword, $fromdb);
            } else {
                return false;
            }
    ...
    {
        if ($this->is_configured() === false) {
            throw new moodle_exception('auth_vnjpcantconnect', 'auth_vnjp');
        }
    ...
    function sync_users(progress_trace $trace, $do_updates = false)
    {
        ...
        if (!empty($this->config->removeuser)) {
            ...
            if (!empty($remove_users)) {
                $trace->output(get_string('auth_vnjpuserstoremove', 'auth_vnjp', count($remove_users)));

                foreach ($remove_users as $user) {
                    if ($this->config->removeuser == AUTH_REMOVEUSER_FULLDELETE) {
                        delete_user($user);
                        $trace->output(get_string('auth_vnjpdeleteuser', 'auth_vnjp', array('name' => $user->username, 'id' => $user->id)), 1);
                    } else if ($this->config->removeuser == AUTH_REMOVEUSER_SUSPEND) {
                        $updateuser = new stdClass();
                        $updateuser->id = $user->id;
                        $updateuser->suspended = 1;
                        user_update_user($updateuser, false);
                        $trace->output(get_string('auth_vnjpsuspenduser', 'auth_vnjp', array('name' => $user->username, 'id' => $user->id)), 1);
                    }
                }
            }
            unset($remove_users);
                ...
                if ($update_users = $DB->get_records_sql($sql, $params)) {
                    ...
                    foreach ($update_users as $user) {
                        if ($this->update_user_record($user->username, $updatekeys)) {
                            $trace->output(get_string('auth_vnjpupdatinguser', 'auth_vnjp', array('name' => $user->username, 'id' => $user->id)), 1);
                        } else {
                            $trace->output(get_string('auth_vnjpupdatinguser', 'auth_vnjp', array('name' => $user->username, 'id' => $user->id)) . " - " . get_string('skipped'), 1);
                        }
                    }
                    unset($update_users);
                }
        ...
        if (!empty($add_users)) {
            $trace->output(get_string('auth_vnjpuserstoadd', 'auth_vnjp', count($add_users)));
            ...
            foreach ($add_users as $user) {
                ...
                if ($this->config->removeuser == AUTH_REMOVEUSER_SUSPEND) {
                    if ($olduser = $DB->get_record('user', array('username' => $username, 'deleted' => 0, 'suspended' => 1,
                        ...
                        $trace->output(get_string('auth_vnjpreviveduser', 'auth_vnjp', array('name' => $username,
                            'id' => $olduser->id)), 1);
                        continue;
                    }
                }
                ...
                if ($collision = $DB->get_record_select('user', "username = :username AND mnethostid = :mnethostid AND auth <> :auth", array('username' => $user->username, 'mnethostid' => $CFG->mnet_localhost_id, 'auth' => $this->authtype), 'id,username,auth')) {
                    $trace->output(get_string('auth_vnjpinsertuserduplicate', 'auth_vnjp', array('username' => $user->username, 'auth' => $collision->auth)), 1);
                    continue;
                }
                try {
                    $id = user_create_user($user, false); // It is truly a new user.
                    $trace->output(get_string('auth_vnjpinsertuser', 'auth_vnjp', array('name' => $user->username, 'id' => $id)), 1);
                } catch (moodle_exception $e) {
                    $trace->output(get_string('auth_vnjpinsertusererror', 'auth_vnjp', $user->username), 1);
                    continue;
                }
    ...
    function user_exists($username)
    {
        ...
        if (!$rs) {
            print_error('auth_vnjpcantconnect', 'auth_vnjp');
        } else if (!$rs->EOF) {
    ...
    function update_user_record($username, $updatekeys = false)
    {
        ...
        if (empty($user)) { // trouble
            ...
            print_error('auth_vnjpusernotexist', 'auth_vnjp', $username);
            die;
        }
    ...
    function validate_form($form, &$err)
    {
        if ($form->passtype === 'internal') {
            ...
            set_config('changepasswordurl', '', 'auth/vnjp');
        }
    }
    ...
    function process_config($config)
    {
        ...
        // Save settings.
        set_config('host', $config->host, 'auth/vnjp');
        set_config('type', $config->type, 'auth/vnjp');
        set_config('sybasequoting', $config->sybasequoting, 'auth/vnjp');
        set_config('name', $config->name, 'auth/vnjp');
        set_config('user', $config->user, 'auth/vnjp');
        set_config('pass', $config->pass, 'auth/vnjp');
        set_config('table', $config->table, 'auth/vnjp');
        set_config('fielduser', $config->fielduser, 'auth/vnjp');
        set_config('fieldpass', $config->fieldpass, 'auth/vnjp');
        set_config('passtype', $config->passtype, 'auth/vnjp');
        set_config('extencoding', trim($config->extencoding), 'auth/vnjp');
        set_config('setupsql', trim($config->setupsql), 'auth/vnjp');
        set_config('debugauthdb', $config->debugauthdb, 'auth/vnjp');
        set_config('removeuser', $config->removeuser, 'auth/vnjp');
        set_config('changepasswordurl', trim($config->changepasswordurl), 'auth/vnjp');
